...
const POKEMON_IDS_BY_EGG_GROUP = PersonalTable.reduce(
  createPokemonByEggGroupMap,
  []
);
...
function getEggGroupViaPokemonId(pokemonId = 0) {
  if (
    ...
    pokemonId < 0 ||
    pokemonId > PersonalTable.length
  )
    throw new Error(`Bad pokemonId: ${pokemonId}`);

  const pokemonDetails = PersonalTable[pokemonId];
...
function getEggGroupNameById(eggGroupId = 0) {
  if (
    !Number.isInteger(eggGroupId) ||
    eggGroupId < 0 ||
    eggGroupId > HIGHEST_EGG_GROUP_ID
  )
